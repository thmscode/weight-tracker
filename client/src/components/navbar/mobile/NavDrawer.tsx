...
const NavDrawer: React.FC<Props> = ({ open, fn }) => {
        ...
        <List>
          ...
          {isAuthenticated ?
            <>
              ...
              <AuthItem key='logout' label='logout' fn={() => logout({ logoutParams: { returnTo: window.location.origin } })} />
            </> :
            <AuthItem key='login' label='Login' fn={() => loginWithRedirect()} />}
        </List>
      </Box>
    </Box>
  )
